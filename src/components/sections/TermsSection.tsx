    ...
    <section className="py-32 px-6 relative overflow-hidden">
      ...
      >
        ...
        <Para>
          ...
          BlinkLife
          移动应用程序及相关服务（以下简称"本服务"）所订立的协议。请您在使用本服务前仔细阅读并充分理解本协议的全部内容。一旦您注册、登录或以其他方式使用本服务，即表示您已阅读、理解并同意接受本协议的约束。
        </Para>
        <Para>
          如果您不同意本协议的任何条款，请勿注册、登录或使用本服务。本协议适用于中华人民共和国法律。
        </Para>

        <Heading>二、账号注册与管理</Heading>
        <List
          items={[
            "您可以通过手机号码或电子邮箱注册 BlinkLife 账号。您应确保注册信息的真实性、准确性和完整性。",
            "您有责任妥善保管账号和密码，因账号密码保管不善导致的一切损失由您自行承担。",
            "您的账号仅限您本人使用，未经我们书面同意，您不得以任何方式转让、赠与或让他人使用您的账号。",
            "如果您发现账号存在安全问题，请立即联系我们。我们有权在合理怀疑账号安全受到威胁时，暂停或终止该账号的使用。",
            "您可以随时申请注销您的账号。账号注销后，相关数据将按照我们的《隐私政策》进行处理。",
          ]}
        />
        ...
        <List
          items={[
            ...
            "云端同步：将视频和打点数据同步至云端，实现跨设备访问（部分功能可能需要付费订阅）。",
          ]}
        />
        <Para>
        ...
        />
        <Para>
          如果我们发现您违反上述行为规范，我们有权采取警告、限制功能、暂停或终止账号等措施，并保留追究法律责任的权利。
        </Para>
        ...
        <List
          items={[
            ...
            "您使用本服务创作的视频内容的知识产权归您所有。您授予我们在提供服务所必需的范围内使用这些内容的有限许可（例如云端存储和传输）。",
            "未经我们书面许可，您不得复制、修改、分发或以其他方式使用本应用的任何内容或商标。",
          ]}
        />

        <Heading>六、付费服务</Heading>
        <Para>
          本应用的基础功能免费提供。我们可能会推出付费增值服务（如云存储空间升级、高级模板、AI
          分析功能等）。付费服务的具体内容、价格和规则将在应用内明确展示。
        </Para>
        <List
          items={[
            "您应通过应用内指定的支付渠道完成付款；",
            "订阅服务将按照您选择的周期自动续费，除非您在到期前取消；",
            "已购买的服务，除法律另有规定外，不予退款。",
          ]}
        />

        <Heading>七、免责条款</Heading>
        <List
          items={[
        ...
        <Heading>八、服务变更与终止</Heading>
        <List
          items={[
            "我们有权根据业务发展需要，对本服务进行更新、升级或功能调整，并将以合理方式通知您。",
            "您可以随时停止使用本服务并注销账号。",
            "如果您严重违反本协议的规定，我们有权立即终止向您提供服务，且无需事先通知。",
            "服务终止后，我们不再承担向您继续提供服务或保留数据的义务，但法律法规另有规定的除外。",
          ]}
        />

        <Heading>九、隐私保护</Heading>
        <Para>
